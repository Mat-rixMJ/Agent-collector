...
def get_youtube_transcript(video_url: str) -> dict:
    ...
    if os.getenv("DEMO_MODE") == "true":
        return {"data": [{"text": "Mock transcript generated for testing offline."}]}
    ...
    try:
        ...
        print(f"  Apify transcript scraper failed for {video_url}: {e}.")
...
def rag_web_search(query: str, max_results: int = 5) -> list[dict]:
    ...
    if os.getenv("DEMO_MODE") == "true":
        ...
        content = f"Mock general market research results for: {query}."
